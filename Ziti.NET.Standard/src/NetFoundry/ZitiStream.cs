...
namespace NetFoundry
{
    ...
    public class ZitiStream : Stream
    {
        ...
        private AfterZitiDataWritten azdw;
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            if (current != null)
            {
                ...
                {
                    if(NetFoundry.Ziti.OutputDebugInformation) Debug.Write("reading stream starts. responses size: " + conn.responses.Count);
                    if (conn.responses.TryTake(out byte[] response, 500))
                    {
                        if(NetFoundry.Ziti.OutputDebugInformation) Debug.Write("read from conn.responses size: " + conn.responses.Count);
                    ...
                    else
                    {
                        ...
                        Ziti.Debug("yielding thread in read");
                        Task.Yield();
                    }
                }
            }
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            ...
            azdw = (IntPtr nf_connection, int status, GCHandle write_ctx) =>
            {
                lock (this)
                {
                    Ziti.Debug("Unlocking write lock");
                    Monitor.Pulse(this);
                }
            };

            Ziti.Write(conn.nf_connection, buffer, count, azdw, ZitiIdentity.NO_CONTEXT/*GCHandle*/);
            lock(this)
            {
                Ziti.Debug("blocking until write is flushed to wire");
                Monitor.Wait(this);
            }
        }
