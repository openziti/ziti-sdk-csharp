...
*/
        ...
        private readonly readonly object _loopLock = new object();
...
After:
			var zitiOptions = Configure(refreshInterval); //use default refresh interval if not supplied
*/
            var zitiOptions = Configure(refreshInterval); //use default refresh interval if not supplied
...
After:
			...
			await runlock.KeepAlive(zitiOptions);
			GC.KeepAlive(zitiOptions);
		}
*/
            }
...
After:
			var cfgs = NAPI.ToPtr(InitOpts.ConfigurationTypes);
*/
            var cfgs = NAPI.ToPtr(InitOpts.ConfigurationTypes);
...
After:
			var result = Native.API.ziti_init_opts(ziti_opts_ptr.Ptr, Loop.nativeUvLoop);
*/
            var result = Native.API.ziti_init_opts(ziti_opts_ptr.Ptr, Loop.nativeUvLoop);
            if (result != 0) {
...
After:
			var type = Native.API.z4d_event_type_from_pointer(ziti_event_t);
*/
            var type = Native.API.z4d_event_type_from_pointer(ziti_event_t);
            switch (type) {
...
After:
					...
					var ztapi = Marshal.PtrToStringUTF8(ptr);
*/
                    var v = Marshal.PtrToStructure<ziti_version>(vptr);
                    var ptr = Native.API.ziti_get_controller(ziti_context);
                    var ztapi = Marshal.PtrToStringUTF8(ptr);
...
After:
						var zid = Marshal.PtrToStructure<ziti_identity>(idPtr);
*/
...
After:
						IdentityNameFromController = zid.name;
*/
...
After:
					var serviceEvent = new ZitiServiceEvent(ziti_context) {
*/
...
After:
					var ziti_mfa_event = Marshal.PtrToStructure<Native.ziti_mfa_event>(ziti_event_t);
*/
                        var zid = Marshal.PtrToStructure<ziti_identity>(idPtr);
...
After:
					var zitiMFAEvent = new ZitiMFAEvent()
*/
...
After:
					var ziti_api_event = Marshal.PtrToStructure<Native.ziti_api_event>(ziti_event_t);
*/
...
After:
					var zitiAPIEvent = new ZitiAPIEvent()
*/
                        name = zid.name;
                        IdentityNameFromController = zid.name;
...
After:
				var bkpConfigFileName = InitOpts.IdentityFile + ".bak";
				File.Move(InitOpts.IdentityFile, bkpConfigFileName);
*/
...
After:
                var json = JsonSerializer.Serialize<Native.IdentityFile>(nid);
				File.WriteAllText(InitOpts.IdentityFile, json);
				Logger.Debug("Created new config file {0}", InitOpts.IdentityFile);
*/
...
After:
			OpenZiti.Native.API.ziti_set_enabled(WrappedContext.nativeZitiContext, enabled);
*/
...
After:
			return OpenZiti.Native.API.ziti_is_enabled(WrappedContext.nativeZitiContext);
*/
...
After:
			OpenZiti.Native.API.ziti_mfa_auth(WrappedContext.nativeZitiContext, code, MFA.AfterMFASubmit, MFA.GetMFAStatusDelegate(this));
*/
...
After:
			OpenZiti.Native.API.ziti_mfa_enroll(WrappedContext.nativeZitiContext, MFA.AfterMFAEnroll, MFA.GetMFAStatusDelegate(this));
*/
...
After:
			OpenZiti.Native.API.ziti_mfa_verify(WrappedContext.nativeZitiContext, code, MFA.AfterMFAVerify, MFA.GetMFAStatusDelegate(this));
*/
...
After:
			OpenZiti.Native.API.ziti_mfa_remove(WrappedContext.nativeZitiContext, code, MFA.AfterMFARemove, MFA.GetMFAStatusDelegate(this));
*/
...
After:
			OpenZiti.Native.API.ziti_mfa_get_recovery_codes(WrappedContext.nativeZitiContext, code, MFA.AfterMFARecoveryCodes, MFA.GetMFAStatusDelegate(this));
*/
...
After:
			OpenZiti.Native.API.ziti_mfa_new_recovery_codes(WrappedContext.nativeZitiContext, code, MFA.AfterMFARecoveryCodes, MFA.GetMFAStatusDelegate(this));
*/
...
After:
			var upIntPtrWrapper = new PrimitiveWrapper<double>();
			var downIntPtrWrapper = new PrimitiveWrapper<double>();
			OpenZiti.Native.API.ziti_get_transfer_rates(WrappedContext.nativeZitiContext, upIntPtrWrapper.Ptr, downIntPtrWrapper.Ptr);
			var upMetrics = new double[1];
*/
...
Before:
			...
			return tm;
After:
			var downMetrics = new double[1];
			Marshal.Copy(downIntPtrWrapper.Ptr, downMetrics, 0, 1);
            var tm = new TransferMetrics {
                Up = upMetrics[0],
                Down = downMetrics[0]
            };
            return tm;
...
After:
			OpenZiti.Native.API.ziti_endpoint_state_change(WrappedContext.nativeZitiContext, woken, unlocked);
*/
...
After:
			NAPI.z4d_ziti_dump_log(WrappedContext.nativeZitiContext);
*/
...
After:
			NAPI.z4d_ziti_dump_file(WrappedContext.nativeZitiContext, fileName);
*/
...
After:
			var version = OpenZiti.Native.API.ziti_get_controller_version(WrappedContext.nativeZitiContext);
			var ver = Marshal.PtrToStructure<ziti_version>(version);
*/
                    }
...
After:
            set {
                removedList = createZitiServiceList(value);
            }
*/
...
After:
        internal IntPtr nativeChangedList {
            set {
                changedList = createZitiServiceList(value);
            }
*/
...
After:
        internal IntPtr nativeAddedList {
            set {
                addedList = createZitiServiceList(value);
            }
        }
        internal IntPtr ziti_ctx { get; }
*/
...
After:
				var zitiService = Native.API.z4d_service_array_get(arr, index);
*/
...
After:
			var servicesList = new List<ZitiService>();
			foreach (var p in array_iterator(nativeServiceArray)) {
...
After:
			foreach (var svc in removedList) {
*/
...
After:
			foreach (var svc in changedList) {
*/
